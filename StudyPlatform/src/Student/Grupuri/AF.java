...
public class AF extends javax.swing.JFrame {
    String email;
    String parola;
    String nume;
    String prenume;
    ...
    String idStud="";
    String numeA="";
    public AF(String email, String parola) {
        initComponents();
        this.email=email;
        this.parola=parola;
        ...
        try{
            ...
            ResultSet resultSet = statement.executeQuery("select * from calendargrup join grup on (grup.id=calendargrup.idG) where idM = (select id from materie where descriere = 'AF') and expirata = 1;");
    ...
    private void programeazaButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from materie where materie.descriere = 'AF'");
            while (resultSet.next()) {
    ...
    private void veziButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            connection = GetConnection.getConnection(this.email, this.parola);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from calendargrup where idG = (Select id from materie where materie.descriere='AF') and expirata=0");
            while (resultSet.next()) {
    ...
    private void loadMes() {
        try {
            BufferedReader reader = new BufferedReader(new FileReader("AF.txt"));
            String line;
    ...
    private void saveMes(String message) {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("AF.txt", true));
