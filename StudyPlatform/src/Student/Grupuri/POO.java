...
public class POO extends javax.swing.JFrame {
    ...
    String idStud="";
    String numeA="";
    public POO(String email, String parola) {
        initComponents();
        this.email=email;
        this.parola=parola;
        ...
        try{
            ...
            ResultSet resultSet = statement.executeQuery("select * from calendargrup join grup on (grup.id=calendargrup.idG) where idM = (select id from materie where descriere = 'POO') and expirata = 1;");
    ...
    private void programeazaButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from materie where materie.descriere = 'POO'");
            while (resultSet.next()) {
    ...
    private void veziButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            connection = GetConnection.getConnection(this.email, this.parola);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from calendargrup where idG = (Select id from materie where materie.descriere='POO') and expirata=0");
            while (resultSet.next()) {
    ...
    private void loadMes() {
        try {
            BufferedReader reader = new BufferedReader(new FileReader("POO.txt"));
            String line;
    ...
    private void saveMes(String message) {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("POO.txt", true));
