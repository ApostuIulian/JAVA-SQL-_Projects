...
public class MES
    ...
    public MES
        ...
        try{
            ...
            ResultSet resultSet = statement.executeQuery("select * from calendargrup join grup on (grup.id=calendargrup.idG) where idM = (select id from materie where descriere = 'MES" +
                    "') and expirata = 1;");
    ...
    private void programeazaButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from materie where materie.descriere = 'MES" +
                    "'");
            while (resultSet.next()) {
    ...
    private void veziButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            connection = GetConnection.getConnection(this.email, this.parola);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from calendargrup where idG = (Select id from materie where materie.descriere='MES" +
                    "') and expirata=0");
            while (resultSet.next()) {
    ...
    private void saveMes(String message) {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("MES" +
                    ".txt", true));
