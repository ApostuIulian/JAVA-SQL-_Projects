...
public class VeziActivitati extends javax.swing.JFrame {
    ...
    public VeziActivitati(String email, String parola) {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("select * from calendar JOIN Activitate on (Calendar.idA=activitate.id) " +
                    "JOIN curs on(activitate.idC=curs.id) JOIN Profesor on(curs.idProf=Profesor.id) where Profesor.email = '"+this.email+"' and startDate = current_DATE();");
            while (resultSet.next()) {
                all=all+resultSet.getString("Activitate.nume")+": Data "+resultSet.getString("Calendar.startDate")+" Ora "+resultSet.getString("calendar.startTime")+"\n";
            }
            activitatiZiArea.insert(all, 0);
            all="";
            ResultSet resultSet1 = statement.executeQuery("select * from calendar JOIN Activitate on (Calendar.idA=activitate.id) " +
                    "JOIN curs on(activitate.idC=curs.id) JOIN Profesor on(curs.idProf=Profesor.id) where Profesor.email = '"+this.email+"'");
            while (resultSet1.next()) {
                all=all+resultSet1.getString("Activitate.nume")+": Data "+resultSet1.getString("Calendar.startDate")+" Ora "+resultSet1.getString("calendar.startTime")+"\n";
            }
